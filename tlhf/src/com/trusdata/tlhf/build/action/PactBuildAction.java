package com.trusdata.tlhf.build.action;
...
import com.trusdata.tlhf.build.bean.SetPactBuildBean;
import com.trusdata.tlhf.build.service.PactBuildService;
 ...
 * 
 */
public class PactBuildAction extends BasePageAction<PactSearchBean> {
	/**
	 * UID
	 */
	...
	public String add(PactBuildBean server) {
		String rtn = "add";
		...
		try {
			...
			if (server == null) {
				...
				insert = SetPactBuildBean.setBean(server);
	...
	public PactBuildBean getBuild() {
		return build;
	}

	public void setBuild(PactBuildBean build) {
		this.build = build;
	}
