...
import com.main.src.common.util.export.word.test1.XWPFHelper;
import com.main.src.common.util.export.word.test1.XWPFHelperTable;

/**
 * @Description 导出word文档
 */
public class TlhfExportWord {
	private XWPFHelperTable xwpfHelperTable = null;
	private XWPFHelper xwpfHelper = null;

	public TlhfExportWord() {
	...
	public void createTableParagraph(XWPFDocument document, int rows, int cols) {
		// xwpfHelperTable.createTable(xdoc, rowSize, cellSize, isSetColWidth,
		// colWidths)
		...
		for (int i = 0; i < rowList.size(); i++) {
			...
			for (int j = 0; j < cellList.size(); j++) {
				XWPFParagraph cellParagraph = cellList.get(j)
						.getParagraphArray(0);
	...
	public void exportCheckWord(Map<String, Object> dataList,
			XWPFDocument document, String savePath) throws IOException {
		XWPFParagraph paragraph = document.getParagraphArray(0);
		XWPFRun titleFun = paragraph.getRuns().get(0);
		titleFun.setText(String.valueOf(dataList.get("TITLE")));
		List<List<Object>> tableData = (List<List<Object>>) dataList
				.get("TABLEDATA");
